...
from weak_learner import Stump
from utils import *
...
class MulticlassDecisionStump:
    ...
    def fit(self, X, Y, W=None, n_jobs=3):
        ...
        stumps = parallelizer(self.find_stump_iterator(n_jobs, sorted_X, sorted_X_idx, Y, W))
        stump = min(stumps)
    ...
    def find_stump_iterator(self, n_jobs, sorted_X, sorted_X_idx, Y, W):
        n_features = sorted_X.shape[1]
        for sub_idx in np.array_split(np.arange(n_features), n_jobs):
            yield self.find_stump(sorted_X[:,sub_idx], sorted_X_idx[:,sub_idx], Y, W, feature_start=sub_idx[0])

    @delayed
    def find_stump(self, sorted_X, sorted_X_idx, Y, W, feature_start=0):
